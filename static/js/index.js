...
(function () {
  ...
  function render(teams, schedule, results) {
    var upcoming = schedule.filter(function (g) { return new Date(g.datetime) >= new Date(); }).length;
    var lastResult = results[results.length - 1];

    var cards = [
      { href: 'teams.html', icon: '👥', title: 'Teams', desc: teams.length + ' team' + (teams.length === 1 ? '' : 's') + ' registered — manage rosters and players.' },
  ...
  document.addEventListener('DOMContentLoaded', function () {
    Promise.all([PCTE.getTeams(), PCTE.getSchedule(), PCTE.getResults()])
      .then(function (values) { render(values[0], values[1], values[2]); })
      .catch(function (err) {
